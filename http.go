...
// exchangeHTTP sends a given query to a given resolver using a DoH POST
// request as described in RFC 8484, and returns the response's body.
// Returns an error if there was an issue sending the request or reading the
// response body.
func exchangeHTTP(q []byte, resolver string) (a []byte, err error) {
	url := resolver
	body := bytes.NewBuffer(q)
	...
	if resp.StatusCode != http.StatusOK {
		err = fmt.Errorf("HTTP server returned with non-OK code %d", resp.StatusCode)
		return
	}
